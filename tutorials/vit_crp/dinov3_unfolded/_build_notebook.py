...
nb.cells = [
    md(
        ...
        "5. HeadConcept atlas at `INSPECTED_LAYER`",
        "6. Q / K / V concept atlases at `INSPECTED_LAYER`",
    ...
    code(
        ...
        "ALPHA, BETA = 0.5, 0.5",
        "",
        "# `INSPECTED_LAYER` and the per-concept layer-path constants are set",
        "# in the build-model cell below (they need `len(model.blocks)`).",
        "",
        "# Block indices used in the cascade (deep → shallow order). Adjust",
        "# if the backbone has a different depth.",
    ...
    code(
        ...
        "",
        "# Layer to inspect for the per-concept atlases. Default: the LAST",
        "# block — closest to the classification head, so most likely to carry",
        "# object-semantic concepts. Override by hand for early-stack",
        "# (texture-ish) views.",
        "INSPECTED_LAYER = len(model.blocks) - 1",
        "print(f'INSPECTED_LAYER = {INSPECTED_LAYER} (last of {len(model.blocks)} blocks)')",
        "",
        "# Explicit per-concept layer paths. These are the actual",
        "# `model.named_modules()` keys the composite + concept code looks up.",
        "# The `backbone.` prefix reflects the Probe wrapper.",
        "HEAD_LAYER = f'backbone.blocks.{INSPECTED_LAYER}.attn.context'",
        "Q_LAYER    = f'backbone.blocks.{INSPECTED_LAYER}.attn.rope_q'",
        "K_LAYER    = f'backbone.blocks.{INSPECTED_LAYER}.attn.rope_k'",
        "V_LAYER    = f'backbone.blocks.{INSPECTED_LAYER}.attn.v_id'",
        "OUT_LAYER  = f'backbone.blocks.{INSPECTED_LAYER}.attn.proj_drop'",
        "REG_LAYER  = f'backbone.blocks.{INSPECTED_LAYER}.attn.proj_drop'  # same module, different concept",
        "CASCADE_LAYER_NAMES = [f'backbone.blocks.{i}.attn.context' for i in CASCADE_LAYERS]",
    ),
    code(
    ...
    code(
        ...
        "print(f'For the atlases below we use INSPECTED_LAYER = {INSPECTED_LAYER}')",
    ...
    md(
        ...
        "",
        "* `HeadConcept` at `INSPECTED_LAYER` — \"images that maximally activate head k\"",
        "* `AttnOutputDimConcept` at `INSPECTED_LAYER` — \"images that activate channel c\"",
        "* `RegisterTokenConcept` at `INSPECTED_LAYER` — \"images that maximally drive a prefix token\"",
        "",
        ...
        "so re-running the cell reuses prior results.",
    ),
    code(
        ...
        "fv_end = len(dataset)",
        "",
        "# Concepts hold a model reference; FeatureVisualization opens its own",
        "# composite.context() per batch so the unfolded submodules are present",
        "# during attribution.",
        "fv_specs = [",
        "    ('head',       HeadConcept(model),           HEAD_LAYER),",
        ...
        "        layer_map={layer: concept}, path=str(cache_path),",
        "        device=DEVICE,",
        "    )",
        "    print(f'indexing {tag} on {layer} over {fv_end} images …')",
    ...
    md(
        ...
        "* **Other layers.** Re-run sections 5-8 with `INSPECTED_LAYER` set to",
