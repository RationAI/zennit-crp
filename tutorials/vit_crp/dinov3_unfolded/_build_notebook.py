...
nb.cells = [
    ...
    code(
        ...
        "",
        "# Build the eval transform. The checkpoint may declare `transform_spec`",
        "# in its payload — required when the published preprocessing differs",
        "# from timm's default (e.g. external pretrained checkpoints trained",
        "# without ImageNet/JFT normalization). If absent, fall back to timm's",
        "# default for this model.",
        "TRANSFORM_SPEC = ckpt.get('transform_spec', 'timm_default')",
        "if 'transform_spec' not in ckpt:",
        "    # This is the silent-failure mode that caused the 0.69 accuracy",
        "    # sanity-eval surprise: an old payload (written before the",
        "    # transform_spec field was added) silently falls back to",
        "    # timm_default, which is wrong for the visinf vit_base checkpoint.",
        ...
        "    # Pulls mean/std/crop_pct/interp from timm's default_cfg for the",
        "    # backbone. For most timm-pretrained ViTs this is right.",
        "    cfg = resolve_data_config({}, model=model.backbone)",
        "    transform = create_transform(**cfg, is_training=False)",
        "    print(f'transform  : timm_default (mean={cfg[\"mean\"]}, std={cfg[\"std\"]}, crop_pct={cfg.get(\"crop_pct\")})')",
        "",
        ...
        "    # crop_pct=0.9 timm default drops it to ~0.85.",
        "    def transform(pil):",
        ...
        "        return t",
        "    print('transform  : visinf_funnybirds_vit_base (no-normalize + bilinear-resize-to-224)')",
        "",
        "else:",
    ...
    code(
        ...
        "#",
        "# `clean_only=True` for funny_birds: the train split mixes ~41%",
        "# part-ablation samples (one or more body parts replaced with",
        "# 'placeholder' renders — these exist to train robustness to missing",
        "# parts but are ambiguous between classes; the test split has 0 of",
        "# them). Including ablations in the FV index would yield concept",
        "# references built on partial birds — misleading. The model is 100%",
        "# accurate on the clean subset of train; 46% on ablated.",
        "_load_kwargs = {",
        "    'funny_birds': dict(split='train', clean_only=True),",
    ...
    md(
        ...
        "",
        "Run the model on a small slice to confirm it predicts well — a near-",
        "chance accuracy here means the checkpoint didn't load correctly (wrong",
        "head shape, missing backbone weights, base/dataset mismatch, or a",
        "transform mismatch between the checkpoint's training pipeline and our",
        "eval pipeline). Bump `EVAL_SUBSET_SIZE` (in section 2) for tighter",
        "estimates.",
        "",
        "**Why a separate test loader?** Published figures (e.g. visinf's",
        "`best_acc1=98`) are on the held-out **test** split. The main `dataset`",
        "above loads `split='train'` (with `clean_only=True` — see the comment",
        "in the previous cell) to give section 9's FV cache a 25× richer",
        "concept pool. The clean-train and test distributions match (both",
        "contain only intact birds, no `'placeholder'` ablations), so the",
        "sanity-eval test number is meaningful AND comparable to the FV-cache",
        "data distribution.",
    ),
    code(
        "from torch.utils.data import DataLoader, Subset",
        "",
        "# Sanity-eval is the canonical 'model healthy?' check, so we run it on",
        "# the dataset's TEST split — that's what published accuracy figures",
        "# refer to. Each dataset module exposes its own naming for the held-",
        "# out split via _eval_load_kwargs.",
        "_eval_load_kwargs = {",
        "    'funny_birds': dict(split='test'),  # test is already 100% clean (no placeholders)",
        "    'dsprites':    dict(target='shape'),  # dsprites is single-split",
        "    'imagenette':  dict(split='val'),",
        "    'imagenet_val_hf': dict(),",
        "}[DATASET]",
        "_eval_dataset = load_dataset(DATASET, transform=transform, **_eval_load_kwargs)",
        "",
        "n_eval = min(EVAL_SUBSET_SIZE, len(_eval_dataset))",
        "_loader = DataLoader(",
        "    Subset(_eval_dataset, list(range(n_eval))),",
        ...
        "        _total   += _y.numel()",
        "_split_label = _eval_load_kwargs.get('split', '(single-split)')",
        "print(f'Sanity-eval top-1 over {_total} samples of {DATASET} [{_split_label}]: {_correct/_total:.4f}')",
