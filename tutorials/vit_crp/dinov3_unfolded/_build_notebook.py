...
nb.cells = [
    ...
    code(
        ...
        "    # in train.py (only `torchvision.transforms.ToTensor` → [0,1]) and",
        "    # then `F.interpolate(x, (224, 224))` (default mode='nearest')",
        "    # inside models/model_wrapper.py::ViTModel.forward. Replicating",
        "    # this exactly recovers the advertised ~98% accuracy on FunnyBirds;",
        "    # the timm-default transform drops the model to ~85% because of",
        "    # the (0.5, 0.5, 0.5)-normalize + bicubic-crop_pct=0.9 mismatch.",
        "    def transform(pil):",
        "        t = TF.to_tensor(pil)[:3]  # drops alpha, keeps [0,1] floats",
        "        t = F.interpolate(t.unsqueeze(0), size=(224, 224)).squeeze(0)",
        "        return t",
        "    print('transform  : visinf_funnybirds_vit_base (no-normalize + nearest-resize-to-224)')",
        "",
        "else:",
