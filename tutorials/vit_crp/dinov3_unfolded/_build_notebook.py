...
nb.cells = [
    ...
    code(
        ...
        "        return t",
        "    # Mirror the no-normalize stats into the backbone's pretrained_cfg",
        "    # so any downstream `resolve_data_config(model.backbone)` consumer",
        "    # (e.g. `viz_unfolded.denormalize`) gets matching mean=0/std=1 and",
        "    # its inverse is identity. Without this, denormalize would shift",
        "    # the already-unnormalized [0,1] tensor up to [0.5, 1.0] using",
        "    # the timm-registered (0.5,0.5,0.5) defaults → pale washed-out",
        "    # display.",
        "    _pcfg = dict(model.backbone.pretrained_cfg)",
        "    _pcfg['mean'], _pcfg['std'] = (0.0, 0.0, 0.0), (1.0, 1.0, 1.0)",
        "    model.backbone.pretrained_cfg = _pcfg",
