...
nb.cells = [
    md(
        ...
        "",
        "**Train your own classifier head** for any of these datasets via the",
        "two-step CLI in `experiments/train_probe.py` — feature cache once,",
        "train any number of head variants on top:",
        "",
        "```",
        "# 1. Cache features (linear head needs cls; attentive needs tokens):",
        "uv run python experiments/train_probe.py cache \\",
        "    {vit_base|vit_dinov3} {funny_birds|dsprites|imagenet_val_hf|imagenette} \\",
        "    --kind {cls|tokens}",
        "",
        "# 2. Train a head on top:",
        "uv run python experiments/train_probe.py train \\",
        "    {vit_base|vit_dinov3} {linear|attentive} \\",
        "    {funny_birds|dsprites|imagenet_val_hf|imagenette}",
        "```",
        "",
        "The notebook below picks one (base, head, dataset) triple (default:",
        "`vit_dinov3 + linear + funny_birds`), rebuilds the model via the same",
        "`build_probe(...)` registry the CLI uses, and runs all the concept",
        "atlases against that probe's class space. Both `linear` and",
        "`attentive` heads are supported — the only thing that changes is",
        "what feature kind the trained head consumes.",
        "",
        "**Notebook structure:**",
        "",
        "1. Setup",
    ...
    code(
        ...
        "from models import BASES, HEADS, build_probe  # the same registry the",
        "                                              # CLI uses, so rebuilding",
    ...
    code(
        ...
        "        f'      {BASE} {DATASET} --kind {head_kind}\\n\\n'",
        "        f'Step 2 — train the head:\\n'",
        "        f'  uv run python experiments/train_probe.py train \\\\\\n'",
        "        f'      {BASE} {HEAD} {DATASET}\\n'",
        "    )",
