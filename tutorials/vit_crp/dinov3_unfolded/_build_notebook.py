...
nb.cells = [
    ...
    code(
        ...
        "",
        "# Explicit per-concept layer paths. We construct the actual",
        "# `model.named_modules()` paths that the composite + concept",
        "# attribution code will look up. The `backbone.` prefix reflects",
        "# the Probe wrapper around the timm backbone — see section 3 for",
        "# the discovery cell that prints these names live.",
        "HEAD_LAYER = f'backbone.blocks.{MID_LAYER}.attn.context'",
        "Q_LAYER    = f'backbone.blocks.{MID_LAYER}.attn.rope_q'",
        "K_LAYER    = f'backbone.blocks.{MID_LAYER}.attn.rope_k'",
        "V_LAYER    = f'backbone.blocks.{MID_LAYER}.attn.v_id'",
        "OUT_LAYER  = f'backbone.blocks.{MID_LAYER}.attn.proj_drop'",
        "REG_LAYER  = f'backbone.blocks.{MID_LAYER}.attn.proj_drop'  # same module, different concept",
    ...
    md(
        ...
        "* `RegisterTokenConcept` at `MID_LAYER` — \"images that maximally drive a prefix token\"",
        "",
