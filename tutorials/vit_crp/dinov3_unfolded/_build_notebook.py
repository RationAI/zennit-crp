...
nb.cells = [
    ...
    md(
        ...
        "estimates.",
        "",
        "**Why a separate test loader?** Published accuracy figures (e.g. the",
        "visinf checkpoint's `best_acc1=98`) are on the held-out test split.",
        "The main `dataset` above loads `split='train'` so the FV cache",
        "(section 9) gets a much richer concept pool — but reporting accuracy",
        "on training data would be misleading. So we open a one-shot test-split",
        "loader for the sanity-eval only.",
    ),
    code(
