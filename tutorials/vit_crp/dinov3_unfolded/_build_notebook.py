...
nb.cells = [
    ...
    code(
        ...
        "# Comment / uncomment ONE option block. Both substitute attention with",
        "# the unfolded form (Eva ↔ EvaAttentionUnfolded, timm ↔",
        "# TimmAttentionUnfolded). The unfolded forms expose identical hookable",
        "# `LRPInspectionLayer` sites (`q_lrp_probe`, `k_lrp_probe`,",
        "# `v_lrp_probe`) so the same concept code path covers both backbones.",
        "",
    ...
    code(
        ...
        "    )",
        "    if has_index:",
        "        print(f'skip {tag}: index present at {cache_path}')",
        "    else:",
        "        print(f'index {tag} → {len(layers)} layer(s) over {len(dataset)} imgs ...')",
    ...
    md(
        ...
        "  `fv.run(...)`. Cache lives at `data/fv_cache_dinov3_unfolded/<tag>`",
