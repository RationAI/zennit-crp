...
nb.cells = [
    ...
    code(
        ...
        "        device=DEVICE,",
        "    )",
        "    # Skip re-indexing if a previous run already wrote *_data.npy for this",
        "    # concept tag. get_max_reference reads the .npy files lazily from disk,",
        "    # so the constructed FV object is fully usable without calling .run().",
        "    # Wipe the tag folder (or set FV_CACHE_DIR to a new path) to force",
        "    # a fresh rebuild.",
        "    existing = list(cache_path.rglob('*_data.npy')) if cache_path.is_dir() else []",
        "    if existing:",
        "        print(f'skipping {tag}: {len(existing)} index file(s) already at {cache_path}')",
        "    else:",
