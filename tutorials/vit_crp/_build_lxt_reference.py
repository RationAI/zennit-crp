...
nb.cells = [
    md(
        ...
        "  `monkey_patch_zennit()` +",
        "  `monkey_patch(vit_torch.vision_transformer, cp_LRP)`).",
        ...
        "the same input image (``tutorials/images/lizard.jpg``), and a",
        "zennit ``LayerMapComposite`` with ``Epsilon(1e-6)`` on Linear /",
        "Conv2d. Both paths use the gradient×input convention via",
        "`monkey_patch_zennit()`.",
        "",
    ...
    code(
        ...
        "from lxt.efficient.models import vit_torch",
        "",
        "monkey_patch(vit_torch.vision_transformer, patch_map=vit_torch.cp_LRP, verbose=True)",
        "print('LXT vit_torch.cp_LRP class-patches applied')",
    ),
    code(
        "# Same Epsilon/Pass layer_map as Path A. NO canonizers — the",
        "# class-level patches from monkey_patch(vit_torch, cp_LRP) cover",
        "# GELU / LayerNorm / MultiheadAttention.",
        "layer_map_b = [",
        "    (nn.Linear,    Epsilon(epsilon=EPSILON)),",
        "    (nn.Conv2d,    Epsilon(epsilon=EPSILON)),",
        "    (nn.GELU,      Pass()),",
