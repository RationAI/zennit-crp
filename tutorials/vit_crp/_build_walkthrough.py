...
nb.cells = [
    ...
    code(
        ...
        "# Two pieces: a `layer_map` mapping `nn.Module` classes to LRP rules,",
        "# and a `canonizers` list of structural rewrites / per-tensor backward",
        "# overrides. zennit's `LayerMapComposite` wires them together. Edit",
        "# any line below — comment out a canonizer, swap a rule, change α/β —",
        "# and rerun this cell to experiment.",
        ...
        "ALPHA, BETA = 0.5, 0.5",
        "",
        "# ─── LAYER MAP — per-module-class LRP rule ──────────────────────────",
        "# Pass() = pure identity rule (relevance flows through unchanged).",
        "layer_map = [",
        "    (nn.Linear,    Epsilon(epsilon=EPSILON)),",
        "    (nn.Conv2d,    Epsilon(epsilon=EPSILON)),",
        "    (nn.GELU,      Pass()),",
        "    (nn.LayerNorm, Pass()),    # ← Identity rule on whole LayerNorm (AttnLRP Prop. 3.4)",
        "    (nn.Dropout,   Pass()),",
        "    (nn.Identity,  Pass()),",
        "]",
        "",
        "# ─── CANONIZERS — structural rewrites + per-tensor backward rules ───",
        "# Comment out any line to drop that piece of the recipe.",
        "canonizers = [",
        "    # Stop-grad-on-std implementation of the LayerNorm rule. Comment",
        "    # OUT to fall back to the layer_map's Pass() alone — the pure",
        "    # Identity treatment (Prop. 3.4 with Taylor reference 0). Bump",
        "    # EXPERIMENT_TAG (section 2) when switching so the resulting FV",
        "    # index lands in a separate cache dir.",
        "    LayerNormForwardCanonizer(),",
        "",
        ...
        "    BilinearMatmulAlphaBetaCanonizer(alpha=ALPHA, beta=BETA, epsilon=EPSILON),",
        "    SoftmaxIdentityCanonizer(),",
        "    ScaleByConstantIdentityCanonizer(),",
        "]",
        "",
    ...
    md(
        ...
        "* **`SoftmaxIdentityCanonizer` / `ScaleByConstantIdentityCanonizer`**",
        "  are harmless extras under CP-LRP — both ops are downstream of",
        "  the detached Q,K and have no grad anyway. Kept for symmetry.",
        "",
    ...
    code(
        "# === COMPOSITE — LXT paper recipe (CP-LRP attention + γ-rule linears) ===",
        ...
        "",
        "# ─── LAYER MAP (LXT) ────────────────────────────────────────────────",
        "# CP-LRP attention: Q and K probes get StopGradient (zero backward",
        "# relevance), V probe stays at Pass (identity). Class-based routing",
        "# replaces the previous name-filtered canonizer — leaves the door",
        "# open for any other per-role rule (e.g. ε on Q only) via layer_map.",
        "lxt_layer_map = [",
        ...
        "    # VInspectionLayer is not listed → falls through to nn.Identity (Pass).",
        "    (nn.Identity,         Pass()),",
        "]",
        "",
        "# ─── CANONIZERS (LXT) ───────────────────────────────────────────────",
        "# No BilinearMatmulAlphaBetaCanonizer — CP-LRP relies on Q,K stop-grad",
        "# + autograd default backward on `weights @ v`.",
        "lxt_canonizers = [",
        "    # Per-element rules (same as the main composite).",
        "    LayerNormForwardCanonizer(),                                 # stop-grad std",
        "    GELUIdentityRuleCanonizer(epsilon=LXT_EPSILON),              # LXT identity",
        "    DropoutPassthroughCanonizer(),",
        "    # Residuals (extrapolated; see note above).",
        "    TimmBlockResidualCanonizer(residual_rule='ratio'),",
        "    EvaBlockResidualCanonizer(residual_rule='ratio', layerscale_uniform=True),",
        "    # Attention substitution → installs Q/K/V inspection probes",
        "    # so the layer_map's Q/K → StopGradient routing takes effect.",
        "    EvaAttentionSubstitutionCanonizer(block_indices=None),",
        "    TimmAttentionSubstitutionCanonizer(block_indices=None),",
        "    # Harmless under CP-LRP (Q,K stop-grad already kills these grads),",
        "    # kept for explicit semantic parity with the main composite.",
        "    SoftmaxIdentityCanonizer(),",
        "    ScaleByConstantIdentityCanonizer(),",
        "]",
        "",
        ...
        "",
        "# To evaluate LXT side-by-side with the main composite, uncomment:",
        "# composite = composite_lxt",
        "# EXPERIMENT_TAG = 'lxt'   # keep FV refs separate (see section 5)",
    ),
