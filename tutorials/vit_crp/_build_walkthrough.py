...
nb.cells = [
    ...
    code(
        ...
        "# Pick one (base, head, dataset). The cell below resolves a checkpoint",
        "# automatically — prefers the most recent OneCycle finetune run under",
        "# `data/runs/finetune_<base>_<dataset>/<ts>/best.pt`, falls back to the",
        "# legacy probe checkpoint `data/<base>_<head>_probe_<dataset>.pt`.",
        ...
        "# Datasets: 'funny_birds' | 'dsprites' | 'colored_mnist' | 'imagenet_val_hf' | 'imagenette'.",
        "BASE = 'vit_small'",
        "HEAD = 'linear'",
        "DATASET = 'funny_birds'",
        "",
        ...
        "_finetune_runs = sorted(_glob.glob(str(DATA_ROOT / 'runs' / f'finetune_{BASE}_{DATASET}' / '*' / 'best.pt')))",
        "_probe_path = DATA_ROOT / f'{BASE}_{HEAD}_probe_{DATASET}.pt'",
        "if _finetune_runs:",
    ...
    md(
        ...
        "",
        "```",
        "data/fv_cache/<base>_<head>_<dataset>/<tag>/",
    ...
    code(
        ...
        "FV_CACHE_DIR = REPO_ROOT / 'data' / 'fv_cache' / f'{BASE}_{HEAD}_{DATASET}'",
    ...
    md(
        ...
        "  `fv.run(...)`. Cache lives at `data/fv_cache/<base>_<head>_<dataset>/<tag>`",
